...
export function registerRoutes(app: Express) {
  ...
  app.get("/api/tenants/:tenantId/billing", async (req, res) => {
    try {
      const tenantId = req.params.tenantId;
      const billingRecords = await storage.getTenantBillingRecords(tenantId);
  ...
  app.get("/api/tenants/:tenantId/outstanding-balance", async (req, res) => {
    try {
      const tenantId = req.params.tenantId;
      const outstandingBalance = await storage.calculateOutstandingBalance(tenantId);
      res.json({ tenantId, outstandingBalance });
    } catch (error) {
  ...
  app.post("/api/billing-records/:id/payment", async (req, res) => {
    try {
      const { id } = req.params;
      const { paymentAmount, paymentMethod } = req.body;
      const billingRecord = await storage.recordPayment(id, paymentAmount, paymentMethod);
      res.json(billingRecord);
    } catch (error) {
  ...
  app.post("/api/generate-monthly-billing", async (req, res) => {
    try {
      await storage.checkAndGenerateMonthlyBilling();
      res.json({ success: true, message: "Monthly billing generation completed" });
    } catch (error) {
